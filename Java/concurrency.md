...
- 6 原子变量类。`java.util.concurrent.atomic` 包
    - “原子变量是一种更好的 volatile”，因为基于非阻塞算法实现（基于 `CAS` 这种更底层的并发原语），性能更高
- 7 `ThreadLocal`
...
> 1、编译出的指令顺序，不一定等于源码的顺序；2、CPU 实际执行的指令顺序，不一定等于编译出的指令顺序；3、变量的值可能存在寄存器或者高速缓存中，不一定和内存中的值一样，这会影响线程读取到正确的值
...
- 线程封闭（Thread Confinement）：解决对象 Escape 导致的线程安全隐患，如：对状态进行封装、用 `ThreadLocal` 实现封闭
