...
```shell
ls > output.txt # 将 stdout 重定向到文件（每次都覆写整个文件）

> output.txt # 清空文件

ls >> output.txt # 将 stdout 追加到文件内容中

ls 2> err.log # 将 stderr 重定向到文件
...
变量分为内置变量（例如：`$PATH`）和自定义变量。内置变量又称环境变量
